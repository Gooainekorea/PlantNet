...
이에 맞춰 확장 진행중이며 현재 코드 리팩터링작업을 활발히 진행하고 있습니다.
...
초기프로토타입: ai모델 업데이트및 모듈화 완료후 esp미니로 기기구현. 사용자가 찍은 식물에 대한 환경을 검색해서 서버에 보내면 서버가 데이터검색해 환경세팅값을 보내고 esp가 값을 받아 환경을 제어하고 주기적/예외상황발생시 데이터값을 서버에 전송.
