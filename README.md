...
이떄 AI써서 한다고 했다가 당시 내 능력 넘어서서 리눅스 깔린 노트북에서 작업했는데 씨메이커로 값이 어디서 어디로 넘어가는지 한줄한줄 찍어보고 왜 제대로 안넘어가는지 찾아봐야 헀음. 쓰레드 써야하는거알고 파이썬으로 쓰레드 배우고 있다면서 멋모르고썼다가 데드락 걸려서 고생했는데 ㅈㄴ.. 전혀안남김
...
정처기때 왜 개발프로세스나 용어나 문서작업 배우는지 이해안갔는데 그게 절반임. 보면서 왜 코딩테스트처럼 안하는지 의아했는데 그것만큼 중요했던거였음
