...
---
# 아래서 부터는 의식의 흐름으로 씀, 오늘내에 정리하고 삭제하기-주말내로하겠다. 누가 주말에 이걸보겠어?
...
4. 죄다 섞기.
    ...
    이때 병목현상 해결하고 가중치 감쇠로 모델학습 성공해서 자신감 만땅이었음
      - 예제코드 보다가 답답해서 GPU쓰려고 환경설정한 나라면
      - 자원소모 확인하면서 병목현상 체크하고 확인한 나라면!
...
이거 전시환경에서 노트북에서 돌려야했음. 그래서 노트북에다가는 웹어플리케이션만 돌림 
...
5. python도 버전관리가 중요함. 
  - 근데 난 초반에 자바와 이클립스 버전문제때문에 안돌아간적 있었음 이때 오라클이 누구랑 싸워서 제크리타인지 뭔지로 바꿨다는데 이때 영향인지 자바 별로 안좋아함
...
6. 이미지 분석 모델의 추론과정은 컴퓨터 자원을 많이씀. 
  ...
  - 근데 이걸로 java와 이클립스, python을 다 돌린다고?
...
# 향후계획 - 이걸로 스마트 테라리움 만들꺼임. 
- 학교서 스마트팜 키트 있어서 두개 받아서 뜯어봄. 
  - 온습도 센서 하나가 녹슬었는데 측정에는 문제 없더라.
  - ESP32인데 보드가 마음에 안들음. 내 손바닥만함. ESP32 wroom만 뜯어다 따로 연결해야할 판임
- 알리서 ESP32 싸다고 샀는데 8266옴. wroom이라고 팔고 있었음. 모르면 등쳐먹으려는거 같아서 열받아서 환불함.
- 보드보다 이것저것삼. 테라리움 작게 만들면 귀여울꺼같아서 ESP32 mini도 사봤는데 가성비랑 크기 좋은듯. 
- 크리스마스 전에 AI 테라리움 여러개 만들어서 주변에 선물해줘야겠음.
  - 서버랑 모델있으니까 1주일정도 소프트웨어랑 펌웨어 작업하면 어플리케이션으로 조작가능하게 SW만들어서 하루에 3개씩은 조립가능하지 않을까?
  - 카메라연동/어플리케이션 구현한다음 기존 프로그래밍 손보면 가능할듯. 어짜피 로직은 있자나 아님 신년선물로 주지뭐

- 할 수 만 있다면 AI서버를 따로 만들고 싶음.
- STM32로 센서값이랑 영상처리하고 ESP가 백엔드가 되서 

요즘 인터넷에서 칩보고 보드 보면서 성능이랑 가격비교하는데 재미들림 STM32 가성비 좋은거같음

이걸로 사업 가능할꺼 같은데?
- 나중에 사업계획서 한번 작성 해봐야겠음. 사람들이 안하는것에는 이유가 있겠지만 문서작업은 뭐든 해놓면 좋다는걸 뼈저리게 느낑

- 아 방금 테라리움 확장하면서 이거 모델학습이랑 웹서버랑 나눠서 문서 작업해야했다는걸 깨달음
...
서버로보내려다 서버유지비등 때문에 기기서 모델을 돌릴수는 없을까 고민했는데 칩이 너무 비쌌음....
서버로 보내면 다중접속자 요청처리해야함
아무나 못들어오게도 해줘야함
...
- 사용자 요청시 데이터전달
- 사용자 환경설정 서버에 전달
- 서버에서 받은 환경설정 stm32에 전달
- 일정시간마다 촬영 후 이미지 전처리해 전달

2. stm32- 센서제어

서버 :
- 기기서받은 환경데이터와 이미지 DB에 저장. 시계열데이터 형성
- 기기서 받은 이미지로 식물 식별, 해당 식물에 맞춘 환경 최적화 데이터 검색
- 최적합 환경데이터 기기 전송
- 사용자/추론된 환경설정 저장
...
1. 시각적 요소때문에 모든 제어부와 모터와 물통을 하단에넣고 5면을 보일수있게하려했음
조명은 줄led쓰고 전에 사둔 미니 가습기 분해해서 쓰려했음

2. 카메라도 위에 달아야해서 그냥 데이터시트랑 핀맵보고 연결하려함. 근데 고민좀 해봐야할듯
난 OV7607쓰려했음.
그리고 팬 크기도 생각해봐야겠음

# 따로 저장해야 하는데 지금 기록도 못할꺼 같아서 일단 여기다 적는거
## 개발자로 취업준비를 하며
지금 회사 두개 면접봄

그리고 그 회사들가보고 내 포폴을 보고 나를 면접에라도 불러주고 가르쳐준것에 감사함을 느꼈음.

포폴을 열심히 만들었다고 생각했는데 아무것도 안적혀있었고 모르는사람이 보면 성의 없는것처럼 느껴졌음

나는 올해 초, 춘천폴리텍 진학전까지 IT분야가 프론트-백-DB로만 나눠진줄 알았음.

AI는 다른직군인줄 알았고 임베디드는 뭔지도 몰랐었음
- 아마 서울북부기술교육원에서도 내가 이정도로 이쪽직군을 모르는건 몰랐을꺼임. 거긴 직종을 고르라고 하진 않았으니까.

그래서 여기와서 어디로 취업할지 고르라고 했을때 설명듣고 맨붕왔었음

서버개발자만 있는게 아니라고? 프론트 아니면 전부 백엔드 아닌가?

그리고 진짜 인공지능을 가르켜주는거임. 그냥 붙여논곳들이 많아서 프롬프트 작상법 같은거 알려주는줄 알았는데 인공지능 구조랑 레이어 통과시 데이터가 어떻게 변하는지 알려줌.

처음 회사 갔을때 밤새서 고민했는데 대체 뭘 설명하란건지 모르겠는거임

피곤하고 긴장한 상태에서 가서 내가 뭐라했냐면... SPI안썼다고함.

첫번쨰 회사에서 이것저것 알려주시며 이슈사항 기재하라고함. 

다행히 최근이고 내가 프로젝트하면서 여기저기 텍스트로 남긴 메모가 있었지만 이떄까지도 뭐가 문제지 모르고 시간없다고 모델학습부분만 로직 정리해서 올림. 그리고 뿌듯해함.

두번쨰 회사 가서는 기기통신질의인줄 알고 통신쪽은 팀장이했다함. 그리고 왜 서버통신관련 질문 안하는지 의아해했음.

그러고 나서야 감잡힘. 이슈사항을 넣으라는게 이렇게 하면 좋았겠지만 난 못했다는걸 말하는게 아니라 하는 과정에서 힘들었는데 왜 힘들었고 그걸 어떻게 해결했는지를 썼어야했음.

지금 팀프로젝트도 다시 정리해야함. 돌겠음. 난 이미 이력서 여기저기 냄 돌아버리겠음

## 문서작업의 중요성
학기초에 C로 미로찾기 한다고 하면서 고생고했는데 과정 안남긴게 후회됨. 

리눅스 깔린 노트북에서 작업했는데 AI써서 한다고 했다가 내 능력 넘어서서 씨메이커로 값이 어디서 어디로 넘어가는지 한줄한줄 찍어보고 왜 제대로 안넘어가는지 동작이 왜 안되는지 찾아봐야 헀음. 쓰레드 써야하는거알고 파이썬으로 쓰레드 배우고 있다면서 멋모르고썼다가 데드락 걸려서 고생했는데 ㅈㄴ.. 전혀안남김

결국 실행파일포기하고 vs로 돌렸는데 전시 노트북에서 안돌아가서 가서 하루종일 만져서 겨우풀었음.

중앙에 점찍고 화면 움직였는데 눈아프다는 평가가 있었음.

자바첫인상이 별로여서 싫어하는것처럼 ai는 학습이 제대로 되었을때 감동받았어서 좋아함

앞으로 모든걸 깃에다 남길꺼임
옵시디언 마크다운 언어에 최적화 되있긴한데 접근성이 좋아야 어디서든 메모남기기 편함. 지금 메모들이 사혼의 조각마냥 흩어져있음. 다 껴맞추면 진짜 소원빌면 이루어질듯

정처기때 왜 개발프로세스나 용어나 작업프로세스 나오는지. 왜 문서작업을 강조하는지 이해안갔는데 그게 절반임. 보면서 왜 코딩테스트처럼 안하는지 의아했는데 코딩을 잘짜는만큼 중요했던거였음.

저 스마트 테라리움을 구현하려면 많은 단계를 거칠것임
저수준 프로그래밍
켈리브레이션
통신보안
모델학습과 DB구조등
그때마다 깃에 메모해야겠음
지금은 아두이노로 구현하지만 테라리움 기초가 잡히면 
ESP-IDF와 STM32CubeIDE로 프로그래밍 해보고싶음
