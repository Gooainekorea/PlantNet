...
- 모션감지 및 배경 변화(이전 화면과의 차이값이 크면) > 식물에 변동이 생김 > 서버에 새로운 식물 분류 요청 (자연스럽게 생기는 식물은 중앙 서버에서 기록된 사진을 비교해 판별)
- 내부 AI모델로 식물 상태 판별 > 비정상 : 서버에 심화 판별 요청 / 정상: SSD에 이전사진 삭제하고 새로운 사진 저장  
