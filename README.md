...
본 프로젝트는 지난 9~10월 진행된 '식물 분류 앱' 학교 프로젝트를 모태로 합니다.  

초기 구상때부터 개인 프로젝트인 IoT기반 식물 관리 기기와 접목시켜 AIoT 기반 식물 관리 시스템을 구축하는 것을 최종 목표로 하고 있었습니다.

이에 맞춰 확장 진행중이며 코드 리팩터링작업을 활발히 진행하고 있습니다.
...
# 현재목표-초기프로토타입제작
ai모델 업데이트및 모듈화 완료후 esp미니로 기기구현. 사용자가 찍은 식물을 서버에 보내면 서버가 식물을 분류하고 해당 식물의 환경설정을 세팅해서 값을 기기에 보내고 esp가 값을 받아 환경을 제어하고 주기적/예외상황발생시 데이터값을 서버에 전송.
...
당시에는 버전 관리 및 문서화가 미흡했으나, 이를 계기로 Git 기반의 체계적인 관리를 하게 되었습니다.
...
### AI 동작 시나리오
1. 엣지로부터 받은 사진요청 판별 및 분석
- 신규 식물 등록
- 정밀 진단

2. 시계열 데이터 기반의 생육 추이 분석 및 식물 종류 정확도 향상
- 사용자별 테라리움의 센서데이터, 사진, 식물군의 종류, 상태등을 결합 분석
...
- 모션감지 및 배경 변화(기준 이미지와 차이값이 크면) > 식물에 변동이 생김 > 서버에 새로운 식물 분류 요청
- 내부 AI모델로 식물 상태 판별 > 비정상 : 서버에 심화 판별 요청 / 정상: 로그 기록 및 썸네일저장 후 대기모드 전환
- 기준 이미지 갱신시 SD카드에 저장, 하루에 한번 기준 이미지 업데이트 및 서버전송
